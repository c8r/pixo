...
const applyTransformToPath = (path, xTransform, yTransform) => {
  ...
  for(let i = 0; i < split.length; i++) {
    const potentialCommand = split[i][0].toLowerCase()
    if(pathGrammar[potentialCommand]) {
      currentCommand = pathGrammar[potentialCommand]
      result.push(split[i][0])
      if(split[i].length > 1) {
        const args = [split[i].substring(1), ...split.slice(i + 1, i + currentCommand.length)].map(toFloat)
        result.push(...currentCommand.conversion(args))
        i = i + currentCommand.length - 1
      }
    }
    else {
      const args = split.slice(i, i + currentCommand.length).map(toFloat)
      result.push(...currentCommand.conversion(args))
      i = i + currentCommand.length - 1
    }
  }
