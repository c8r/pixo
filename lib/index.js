...
const translatePath = (el) => {
  ...
  const [all, configX, configY] = regex.exec(el.properties.transform)
  const x = parseInt(configX) || 0
  const y = parseInt(configY) || 0
  const commands = {
    M: [x, y],
    L: [x, y],
    H: [x],
        ...
        .map((param, i) => {
          ...
          if (!isNaN(asInt)) {
            return (
              asInt +
              (currTransform
                ? currTransform[(i - commandIndex - 1) % currTransform.length]
                : 0)
            )
          }
          currTransform = commands[param]
          commandIndex = i
          return param
        })
        .join(' '),
