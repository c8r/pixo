...
const translateInput = (el, x, y) => ({
  ...el,
  properties: applyTransformsToProps(el.properties, [
    [['x', 'cx'], (val) => toInt(x) + toInt(val)],
    [['y', 'cy'], (val) => toInt(y) + toInt(val)],
  ]),
})

const transformInput = el => {
  const regex = /(translate)\((-?\d*\.?\d+)[' '|',']*(-?\d*\.?\d+)*\)/g
  if(!el.properties.transform) {
    return el
  }
  ...
  const transforms = []
  let match
  while(match = regex.exec(el.properties.transform)) {
    const [ all, transform, ...args ] = match
    transforms.push([transformFuncs[transform], ...args])
  }
